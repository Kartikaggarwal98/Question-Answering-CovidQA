...
# mode = 'train'
train_data = json.load(open(f'covid-qa/covid-qa-train.json'))['data']
dev_data = json.load(open(f'covid-qa/covid-qa-dev.json'))['data']
test_data = json.load(open(f'covid-qa/covid-qa-test.json'))['data']
print (f'data size: train- {len(train_data)}, dev-{len(dev_data)}, test-{len(test_data)}')
...
def convert_to_pandas(data):
    ...
    for did, document in enumerate(data):
        ...
        for qid, qa in enumerate(qas):
            dict_object = {'context':context, 'question':qa['question'],'id':qa['id']}
            
            answer_object = {'answer_start':[],'text':[]}

            for ans in qa['answers']:
                answer_object['text'].append(ans['text'])
                answer_object['answer_start'].append(ans['answer_start'])

            dict_object['answers'] = answer_object
            
            qac_triples.append(dict_object)
    
    print ('Data Size: ',len(qac_triples))
    return qac_triples
    # return pd.DataFrame(qac_triples)


with open('train.json','w') as f:
    json.dump({'data':convert_to_pandas(train_data)},f)
with open('validation.json','w') as f:
    json.dump({'data':convert_to_pandas(dev_data)},f)
with open('test.json','w') as f:
    json.dump({'data':convert_to_pandas(test_data)},f)

ds = datasets.load_dataset('json',data_files={'train':'train.json','validation':'validation.json','test':'test.json'},field='data')#
ds
...
print ("Without adapter parameters to train: ",sum(p.numel() for p in model.parameters() if p.requires_grad))



####### HERE STARTS THE ADAPTER CODE ###########

adap_name = 'covid-qa_adap'  ### this is a new adapter (training from scratch) 
...
print ("With adapter parameters to train: ",sum(p.numel() for p in model.parameters() if p.requires_grad))

####### END THE ADAPTER CODE ###########
...
final_predictions = postprocess_qa_predictions(ds["test"], test_features, raw_predictions.predictions)
...
if squad_v2:
    ...
    formatted_predictions = [{"id": k, "prediction_text": v} for k, v in final_predictions.items()]
references = [{"id": ex["id"], "answers": ex["answers"]} for ex in ds["test"]]
print (metric.compute(predictions=formatted_predictions, references=references))

sorted_dict = {}
for pred in formatted_predictions:
    sorted_dict[pred['id']]=pred['prediction_text']

sorted_dict = dict(sorted(sorted_dict.items())).values()
print (sorted_dict)
with open('predict_test_ft.txt','w') as f:
